#!/bin/bash
#===============================================================================
# Pop!_OS 24.04 LTS NVIDIA — AI/Dev Workstation Auto-Setup v5.0.0
#===============================================================================
...
# Version : 5.0.0 (2026-04-18) — STABLE | Stages 1-26 + Profiles
#===============================================================================
...
# Profile selection
SELECTED_PROFILE="full"  # default: workstation | cluster | ai-dev | full
if [[ -n "${PROFILE:-}" ]]; then
...
# ════════════════════════════════════════════════════════════════════════════
# STAGE 21 — CUDA Toolkit Full (Nsight, nvprof, cuBLAS, cuFFT, cuSOLVER)
# ════════════════════════════════════════════════════════════════════════════
stage21_cuda_full() {
    if [[ "$GPU_DETECTED" == "no" ]]; then
        logWarn "No GPU — skipping CUDA Full"; return 0
    fi
    if ! command -v nvcc &>/dev/null; then
        logWarn "CUDA not installed — run Stage 4 first"; return 0
    fi
    if ! ask_yes_no "Install CUDA Full Stack (Nsight, cuBLAS, cuFFT, cuSOLVER, nvprof)?" "n"; then
        log "CUDA Full stack skipped"; return 0
    fi
    apt install -y \
    ...
    if command -v nsys &>/dev/null; then
        logOk "NVIDIA Nsight Systems installed"
    fi
    if command -v nvprof &>/dev/null; then
        logOk "nvprof available"
    fi
    logOk "CUDA Full Stack installed"
}

# ════════════════════════════════════════════════════════════════════════════
# STAGE 22 — ROS2 Humble (Robot Operating System)
# ════════════════════════════════════════════════════════════════════════════
stage22_ros2() {
    if ! ask_yes_no "Install ROS2 Humble (robotics stack, Gazebo, colcon)?" "n"; then
        log "ROS2 skipped"; return 0
    fi
    if command -v ros2 &>/dev/null; then
        logOk "ROS2 already installed"; return 0
    fi
    locale || apt install -y locales && locale-gen en_US.UTF-8
    apt install -y \
        locales \
        gnupg \
        curl \
        wget
    curl -sSL https://raw.githubusercontent.com/ros/rosdistro/master/ros.key | apt-key add - 2>/dev/null || wget -qO - https://raw.githubusercontent.com/ros/rosdistro/master/ros.key | apt-key add -
    ...
    echo "source /opt/ros/humble/setup.bash" >> "$HOMEDIR/.bashrc"
    logOk "ROS2 Humble installed"
    log "Test: source /opt/ros/humble/setup.bash && ros2 topic list"
}

# ════════════════════════════════════════════════════════════════════════════
# STAGE 23 — k3s Full Stack (Ingress NGINX, MetalLB, cert-manager, ArgoCD)
# ════════════════════════════════════════════════════════════════════════════
stage23_k3s_full() {
    if ! command -v k3s &>/dev/null; then
        logWarn "k3s not installed — run Stage 6 first"; return 0
    fi
    if ! ask_yes_no "Install k3s Full Stack (Ingress NGINX, MetalLB, cert-manager, ArgoCD)?" "n"; then
        log "k3s Full stack skipped"; return 0
    fi
    log "Waiting for k3s API..."
    until kubectl get nodes &>/dev/null; do sleep 2; done
    kubectl apply -f https://raw.githubusercontent.com/kubernetes/ingress-nginx/controller-v1.11.2/deploy/static/provider/cloud/deploy.yaml 2>/dev/null || \
    kubectl apply -f https://raw.githubusercontent.com/kubernetes/ingress-nginx/controller-v1.10.1/deploy/static/provider/host/deploy.yaml
    kubectl wait --namespace ingress-nginx \
        --for=condition=ready pod \
        --selector=app.kubernetes.io/component=controller \
        --timeout=120s 2>/dev/null || logWarn "Ingress NGINX timeout"
    kubectl apply -f https://raw.githubusercontent.com/metallb/metallb/v0.14.3/config/manifests/metallb-native.yaml
    kubectl wait --namespace metallb-system \
        --for=condition=ready pod \
        --selector=app=metallb \
        --timeout=120s 2>/dev/null || logWarn "MetalLB timeout"
    ...
    helm install cert-manager jetstack/cert-manager \
        ...
        --timeout 120s 2>&1 | tail -3
    kubectl apply -f https://raw.githubusercontent.com/argoproj/argo-cd/v2.12.3/manifests/install.yaml
    logOk "Ingress NGINX + MetalLB + cert-manager + ArgoCD deployed"
    log "ArgoCD UI: kubectl port-forward -n argocd svc/argocd-server 30880:443"
    log "ArgoCD password: kubectl -n argocd get secret argocd-initial-admin-secret -o jsonpath='{.data.password}' | base64 -d"
}

# ════════════════════════════════════════════════════════════════════════════
# STAGE 24 — Zsh Tuning (theme, plugins, AI/K8s aliases, Starship)
# ════════════════════════════════════════════════════════════════════════════
stage24_zsh_tuning() {
    if ! command -v zsh &>/dev/null; then
        logWarn "Zsh not installed — run Stage 8 first"; return 0
    fi
    if ! ask_yes_no "Apply Zsh Tuning (Starship, custom aliases, AI/K8s shortcuts)?" "n"; then
        log "Zsh tuning skipped"; return 0
    fi
    if ! command -v starship &>/dev/null; then
        curl -sS https://starship.rs/install.sh | sh -s -- -y 2>/dev/null || \
        sh -c "$(curl -fsSL https://starship.rs/install.sh)" -- -y
    fi
    cat >> "$HOMEDIR/.zshrc" << 'ZSHRC'
...
alias kgs='kubectl get storageclass'
...
ZSHRC
    mkdir -p "$HOMEDIR/.oh-my-zsh/themes" 2>/dev/null || true
    chown -R "$CURRENT_USER:$CURRENT_USER" "$HOMEDIR/.zshrc" 2>/dev/null || true
    logOk "Zsh tuned: Starship + AI/K8s aliases"
    log "Apply: source ~/.zshrc"
}

# ════════════════════════════════════════════════════════════════════════════
# STAGE 25 — KDE Full Customization (theme, widgets, hotkeys, latte-dock)
# ════════════════════════════════════════════════════════════════════════════
stage25_kde_custom() {
    if ! ask_yes_no "Apply KDE Full Customization (Breeze Dark, latte-dock, widgets, hotkeys)?" "n"; then
        log "KDE customization skipped"; return 0
    fi
    apt install -y \
        ...
        konsole \
        2>&1 | tail -3
    sudo -u "$CURRENT_USER" dbus-launch gsettings set org.gnome.desktop.interface gtk-theme 'Breeze Dark' 2>/dev/null || true
    sudo -u "$CURRENT_USER" dbus-launch gsettings set org.gnome.desktop.interface icon-theme 'breeze-dark' 2>/dev/null || true
    if command -v latte &>/dev/null; then
        sudo -u "$CURRENT_USER" latte-dock --configure &
        sleep 2
    fi
    sudo -u "$CURRENT_USER" mkdir -p "$HOMEDIR/.config/autostart"
    cat > "$HOMEDIR/.config/autostart/latte-dock.desktop" << 'LATTE'
[Desktop Entry]
Type=Application
Name=Latte Dock
Exec=latte-dock
Comment=Customized KDE Dock
LATTE
    chown "$CURRENT_USER:$CURRENT_USER" "$HOMEDIR/.config/autostart/latte-dock.desktop"
    sudo -u "$CURRENT_USER" kwriteconfig5 --file kwinrc --group Windows --key Alt+Shift+Alt+L "Electrify All Windows" 2>/dev/null || true
    logOk "KDE customized: Latte Dock + Breeze Dark"
    log "Latte layout: https://store.kde.org/browse?cat=0&ord=Latest"
}

# ════════════════════════════════════════════════════════════════════════════
# STAGE 26 — AI Workstation (vLLM + Text Generation WebUI + Ollama)
# ════════════════════════════════════════════════════════════════════════════
stage26_ai_workstation() {
    if ! ask_yes_no "Install AI Workstation Stack (vLLM, Text Generation WebUI, Ollama)?" "n"; then
        log "AI Workstation stack skipped"; return 0
    fi
    # Ollama (already in Stage 10, but ensure latest)
    if ! command -v ollama &>/dev/null; then
        curl -fsSL https://ollama.com/install.sh | sh 2>&1 | tail -3
    else
        ollama update 2>/dev/null || log "Ollama already up to date"
    fi
    # vLLM
    if [[ "$GPU_DETECTED" == "yes" && "$USE_CUDA" == "yes" ]]; then
        if ! pip3 show vllm &>/dev/null; then
            log "Installing vLLM..."
            pip3 install vllm torch --index-url https://download.pytorch.org/whl/cu124 2>&1 | tail -5
        fi
        logOk "vLLM installed (GPU mode)"
        log "Start vLLM: python3 -m vllm.entrypoints.openai.api_server --model meta-llama/Llama-3.1-8B-Instruct --gpu-memory-utilization 0.9"
    else
        pip3 install vllm 2>&1 | tail -3
        logWarn "vLLM installed (CPU mode — GPU recommended)"
    fi
    # Text Generation WebUI (oobabooga)
    if [[ ! -d "$HOMEDIR/text-generation-webui" ]]; then
        if ask_yes_no "Clone Text Generation WebUI (oobabooga)?" "n"; then
            sudo -u "$CURRENT_USER" git clone --depth=1 https://github.com/oobabooga/text-generation-webui.git "$HOMEDIR/text-generation-webui"
            cd "$HOMEDIR/text-generation-webui"
            sudo -u "$CURRENT_USER" pip3 install -r requirements.txt 2>&1 | tail -3
            logOk "Text Generation WebUI cloned to $HOMEDIR/text-generation-webui"
            log "Start: python3 server.py --model llama-7b.ggmlv3.q4_K_M.bin --listen"
        fi
    fi
    logOk "AI Workstation ready"
    log "Ollama: ollama run llama3 | vLLM: python3 -m vllm.entrypoints.openai.api_server | WebUI: cd $HOMEDIR/text-generation-webui && python3 server.py"
}
...
main() {
    echo "╔══════════════════════════════════════════════════════════════════╗"
    echo "║   Pop!_OS 24.04 — AI/Dev Workstation Setup v5.0.0          ║"
    echo "║   Profile: $SELECTED_PROFILE | GPU: $GPU_DETECTED              ║"
    echo "╚══════════════════════════════════════════════════════════════════╝"
    echo ""
    echo "Profile: $SELECTED_PROFILE | GPU: $GPU_DETECTED"
    echo "User:    $CURRENT_USER | Home: $HOMEDIR"
    echo "Log:     $LOGFILE"
    echo ""
    ...
    # === Profile: workstation | ai-dev ===
    if [[ "$SELECTED_PROFILE" == "workstation" || "$SELECTED_PROFILE" == "ai-dev" || "$SELECTED_PROFILE" == "full" ]]; then
    ...
    # === Profile: cluster | full ===
    if [[ "$SELECTED_PROFILE" == "cluster" || "$SELECTED_PROFILE" == "full" ]]; then
    ...
    if [[ "$SELECTED_PROFILE" == "full" ]]; then
        run_stage 21 "CUDA Full Stack"            stage21_cuda_full
        run_stage 22 "ROS2 Humble"               stage22_ros2
        run_stage 23 "k3s Full Stack"             stage23_k3s_full
        run_stage 24 "Zsh Tuning"                stage24_zsh_tuning
        run_stage 25 "KDE Customization"         stage25_kde_custom
        run_stage 26 "AI Workstation"             stage26_ai_workstation
    fi

    echo ""
    echo "╔══════════════════════════════════════════════════════════════════╗"
    echo "║   ✅ SETUP COMPLETE — v$SCRIPT_VERSION ($SELECTED_PROFILE)                  ║"
    echo "╚══════════════════════════════════════════════════════════════════╝"
    echo ""
    echo "NEXT STEPS:"
    ...
    echo " 11. ROS2 Humble:        source /opt/ros/humble/setup.bash && ros2 topic list"
    echo " 12. ArgoCD:            kubectl port-forward -n argocd svc/argocd-server 30880:443"
    echo ""
    echo "Profiles:"
    echo "  workstation  — base dev + zsh + security + KDE + AI stack"
    echo "  cluster      — k3s + Longhorn + Rook Ceph + MinIO + monitoring"
    echo "  ai-dev       — workstation + CUDA + PyTorch + Ollama + vLLM"
    echo "  full         — all stages (1-26)"
    echo ""
    echo "Run with profile: sudo PROFILE=workstation bash pop-os-setup-v5.sh"
